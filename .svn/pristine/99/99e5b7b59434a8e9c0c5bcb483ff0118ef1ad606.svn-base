...
namespace UWay.Skynet.Cloud.Data.Linq.Expressions
{
    ...
    /// </summary>
    class CrossApplyRewriter : DbExpressionVisitor
    {
            ...
            if (join.Join == JoinType.CrossApply || join.Join == JoinType.OuterApply)
            {
                ...
                else
                {
                        ...
                        && (select.GroupBy == null || select.GroupBy.Count == 0))
                    {
                        ...
                        HashSet<TableAlias> declaredAliases = TableAliasGatherer.Gather(join.Left);
                        referencedAliases.IntersectWith(declaredAliases);
                        if (referencedAliases.Count == 0)
                        {
                            Expression where = select.Where;
                            select = selectWithoutWhere;
                            var pc = ColumnProjector.ProjectColumns(where, select.Columns, select.Alias, TableAliasGatherer.Gather(select.From));
