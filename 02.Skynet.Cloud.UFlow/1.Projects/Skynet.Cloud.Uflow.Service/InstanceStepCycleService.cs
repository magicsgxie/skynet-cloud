...
* 版本号：  V1.0.0.0
* 唯一标识：eae3106f-7f4f-44ee-8427-8eec5742b3bc
* 创建人：  magic.s.g.xie
* 电子邮箱：xiesg@uway.cn
...
namespace   UWay.Skynet.Cloud.Uflow.Service
{
   ...
   /// </summary>
   public class InstanceStepCycleService: IInstanceStepCycleService
   {
      /// <summary>
      ...
      public long Add(InstanceStepCycle  instanceStepCycle)
      {
         ...
         {
            return new InstanceStepCycleRepository(dbContext).Add(instanceStepCycle);
         }
      }
      /// <summary>
      /// 添加存放每一次循环处理的结果，把最后的结果更新到事件表中{存放每一次循环处理的结果，把最后的结果更新到事件表中}对象(即:一条记录
      ...
      public void Add(IList<InstanceStepCycle>  instanceStepCycles)
      {
         using(var dbContext = UnitOfWork.Get(Unity.ContainerName))
         {
            new InstanceStepCycleRepository(dbContext).Add(instanceStepCycles);
         }
      }
      /// <summary>
      ...
      public int Update(InstanceStepCycle  instanceStepCycle)
      {
         using(var dbContext = UnitOfWork.Get(Unity.ContainerName))
         {
            return new InstanceStepCycleRepository(dbContext).Update(instanceStepCycle);
         }
      }
      /// <summary>
      ...
      public int Delete(string[] idArrays )
      {
         using(var dbContext = UnitOfWork.Get(Unity.ContainerName))
         {
            return new InstanceStepCycleRepository(dbContext).Delete(idArrays);
         }
      }
      /// <summary>
      ...
      public InstanceStepCycle GetById(string id)
      {
         using(var dbContext = UnitOfWork.Get(Unity.ContainerName))
         {
            return new InstanceStepCycleRepository(dbContext).GetById(id);
         }
      }
      /// <summary>
      ...
      public DataSourceResult Page(DataSourceRequest request)
      {
         using(var dbContext = UnitOfWork.Get(Unity.ContainerName))
         {
            return new InstanceStepCycleRepository(dbContext).Query().ToDataSourceResult(request);
         }
      }
   }
}
