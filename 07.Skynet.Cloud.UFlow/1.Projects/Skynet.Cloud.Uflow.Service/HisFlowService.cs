...
* 版本号：  V1.0.0.0
* 唯一标识：eca2674c-eb99-44da-838d-07b478064db7
* 创建人：  magic.s.g.xie
* 电子邮箱：xiesg@uway.cn
...
namespace   UWay.Skynet.Cloud.Uflow.Service
{
   ...
   /// 流程实例表服务实现类
   /// </summary>
   public class HisFlowService: IHisFlowService
   {
      /// <summary>
      ...
      public long Add(HisFlow  hisFlow)
      {
         ...
         {
            return new HisFlowRepository(dbContext).Add(hisFlow);
         }
      }
      /// <summary>
      /// 添加流程实例表{流程实例表}对象(即:一条记录
      ...
      public void Add(IList<HisFlow>  hisFlows)
      {
         using(var dbContext = UnitOfWork.Get(Unity.ContainerName))
         {
            new HisFlowRepository(dbContext).Add(hisFlows);
         }
      }
      /// <summary>
      ...
      public int Update(HisFlow  hisFlow)
      {
         using(var dbContext = UnitOfWork.Get(Unity.ContainerName))
         {
            return new HisFlowRepository(dbContext).Update(hisFlow);
         }
      }
      /// <summary>
      ...
      public int Delete(string[] idArrays )
      {
         using(var dbContext = UnitOfWork.Get(Unity.ContainerName))
         {
            return new HisFlowRepository(dbContext).Delete(idArrays);
         }
      }
      /// <summary>
      ...
      public HisFlow GetById(string id)
      {
         using(var dbContext = UnitOfWork.Get(Unity.ContainerName))
         {
            return new HisFlowRepository(dbContext).GetById(id);
         }
      }
      /// <summary>
      ...
      public DataSourceResult Page(DataSourceRequest request)
      {
         using(var dbContext = UnitOfWork.Get(Unity.ContainerName))
         {
            return new HisFlowRepository(dbContext).Query().ToDataSourceResult(request);
         }
      }
   }
}
