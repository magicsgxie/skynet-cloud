/************************************************************************************
* Copyright (c) 2019-07-11 12:03:07 优网科技 All Rights Reserved.
* CLR版本： .Standard 2.x
* 公司名称：优网科技
* 命名空间：UWay.Skynet.Cloud.Uflow.Service.Interface
* 文件名：  UWay.Skynet.Cloud.Uflow.Service.Interface.InstanceFlow.cs
* 版本号：  V1.0.0.0
* 唯一标识：c3497de3-89ae-4a0f-b515-8ca6796358e8
* 创建人：  magic.s.g.xie
* 电子邮箱：xiesg@uway.cn
* 创建时间：2019-07-11 12:03:07 
* 描述：流程实例表 
* 
* 
* =====================================================================
* 修改标记 
* 修改时间：2019-07-11 12:03:07 
* 修改人： 谢韶光
* 版本号： V1.0.0.0
* 描述：
* 
* 
* 
* 
************************************************************************************/



namespace   UWay.Skynet.Cloud.Uflow.Service.Interface
{
   using System;
   using UWay.Skynet.Cloud.Data;

   using UWay.Skynet.Cloud.Request;
   using UWay.Skynet.Cloud.Uflow.Entity;
   using System.Collections.Generic;
   /// <summary>
   /// 流程实例表服务接口类
   /// </summary>
   public interface IInstanceFlowService
   {
      /// <summary>
      /// 添加流程实例表{流程实例表}对象(即:一条记录
      /// </summary>
      long Add(InstanceFlow  instanceFlow);
      /// <summary>
      /// 添加流程实例表{流程实例表}对象(即:一条记录
      /// </summary>
      void Add(IList<InstanceFlow>  instanceFlows);
      /// <summary>
      /// 更新流程实例表{流程实例表}对象(即:一条记录
      /// </summary>
      int Update(InstanceFlow  instanceFlow);
      /// <summary>
      /// 删除流程实例表{流程实例表}对象(即:一条记录
      /// </summary>
      int Delete(string[] idArrays );
      /// <summary>
      /// 获取指定的流程实例表{流程实例表}对象(即:一条记录
      /// </summary>
      InstanceFlow GetById(string id);
      /// <summary>
      /// 获取所有的流程实例表{流程实例表}对象(即:一条记录
      /// </summary>
      DataSourceResult Page(DataSourceRequest request);
   }
}
