/************************************************************************************
* Copyright (c) 2019-07-11 12:02:48 优网科技 All Rights Reserved.
* CLR版本： .Standard 2.x
* 公司名称：优网科技
* 命名空间：UWay.Skynet.Cloud.Uflow.Service.Interface
* 文件名：  UWay.Skynet.Cloud.Uflow.Service.Interface.CfgWorktimeDef.cs
* 版本号：  V1.0.0.0
* 唯一标识：93b4af1c-927c-4542-a536-138fb061cc52
* 创建人：  magic.s.g.xie
* 电子邮箱：xiesg@uway.cn
* 创建时间：2019-07-11 12:02:48 
* 描述：默认日工作时间 
* 
* 
* =====================================================================
* 修改标记 
* 修改时间：2019-07-11 12:02:48 
* 修改人： 谢韶光
* 版本号： V1.0.0.0
* 描述：
* 
* 
* 
* 
************************************************************************************/



namespace   UWay.Skynet.Cloud.Uflow.Service.Interface
{
   using System;
   using UWay.Skynet.Cloud.Data;

   using UWay.Skynet.Cloud.Request;
   using UWay.Skynet.Cloud.Uflow.Entity;
   using System.Collections.Generic;
   /// <summary>
   /// 默认日工作时间服务接口类
   /// </summary>
   public interface ICfgWorktimeDefService
   {
      /// <summary>
      /// 添加默认日工作时间{默认日工作时间}对象(即:一条记录
      /// </summary>
      long Add(CfgWorktimeDef  cfgWorktimeDef);
      /// <summary>
      /// 添加默认日工作时间{默认日工作时间}对象(即:一条记录
      /// </summary>
      void Add(IList<CfgWorktimeDef>  cfgWorktimeDefs);
      /// <summary>
      /// 更新默认日工作时间{默认日工作时间}对象(即:一条记录
      /// </summary>
      int Update(CfgWorktimeDef  cfgWorktimeDef);
      /// <summary>
      /// 删除默认日工作时间{默认日工作时间}对象(即:一条记录
      /// </summary>
      int Delete(string[] idArrays );
      /// <summary>
      /// 获取指定的默认日工作时间{默认日工作时间}对象(即:一条记录
      /// </summary>
      CfgWorktimeDef GetById(string id);
      /// <summary>
      /// 获取所有的默认日工作时间{默认日工作时间}对象(即:一条记录
      /// </summary>
      DataSourceResult Page(DataSourceRequest request);
   }
}
