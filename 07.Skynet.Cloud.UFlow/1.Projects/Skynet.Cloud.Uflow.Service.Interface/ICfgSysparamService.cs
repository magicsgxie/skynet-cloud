/************************************************************************************
* Copyright (c) 2019-07-11 12:03:17 优网科技 All Rights Reserved.
* CLR版本： .Standard 2.x
* 公司名称：优网科技
* 命名空间：UWay.Skynet.Cloud.Uflow.Service.Interface
* 文件名：  UWay.Skynet.Cloud.Uflow.Service.Interface.CfgSysparam.cs
* 版本号：  V1.0.0.0
* 唯一标识：49d35945-5ed4-487f-a9ed-579a6b8185ff
* 创建人：  magic.s.g.xie
* 电子邮箱：xiesg@uway.cn
* 创建时间：2019-07-11 12:03:17 
* 描述：系统参数 
* 
* 
* =====================================================================
* 修改标记 
* 修改时间：2019-07-11 12:03:17 
* 修改人： 谢韶光
* 版本号： V1.0.0.0
* 描述：
* 
* 
* 
* 
************************************************************************************/



namespace   UWay.Skynet.Cloud.Uflow.Service.Interface
{
   using System;
   using UWay.Skynet.Cloud.Data;

   using UWay.Skynet.Cloud.Request;
   using UWay.Skynet.Cloud.Uflow.Entity;
   using System.Collections.Generic;
   /// <summary>
   /// 系统参数服务接口类
   /// </summary>
   public interface ICfgSysparamService
   {
      /// <summary>
      /// 添加系统参数{系统参数}对象(即:一条记录
      /// </summary>
      long Add(CfgSysparam  cfgSysparam);
      /// <summary>
      /// 添加系统参数{系统参数}对象(即:一条记录
      /// </summary>
      void Add(IList<CfgSysparam>  cfgSysparams);
      /// <summary>
      /// 更新系统参数{系统参数}对象(即:一条记录
      /// </summary>
      int Update(CfgSysparam  cfgSysparam);
      /// <summary>
      /// 删除系统参数{系统参数}对象(即:一条记录
      /// </summary>
      int Delete(string[] idArrays );
      /// <summary>
      /// 获取指定的系统参数{系统参数}对象(即:一条记录
      /// </summary>
      CfgSysparam GetById(string id);
      /// <summary>
      /// 获取所有的系统参数{系统参数}对象(即:一条记录
      /// </summary>
      DataSourceResult Page(DataSourceRequest request);
   }
}
