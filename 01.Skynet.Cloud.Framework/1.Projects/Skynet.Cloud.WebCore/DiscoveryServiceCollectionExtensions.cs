...
namespace UWay.Skynet.Cloud.WebCore
{
    ...
    public static class DiscoveryServiceCollectionExtensions
    {
        ...
        private static void AddDiscoveryServices(IServiceCollection services, IServiceInfo info, IConfiguration config, IDiscoveryLifecycle lifecycle)
        {
            ...
            } else if(IsNacosConfigured(config, info))
            {
                ConfigureNacosServices(services, config, info);
                AddNacosServices(services, config, lifecycle);
            }
            else
            {
        ...
        #region NACOS
        private static bool IsNacosConfigured(IConfiguration config, IServiceInfo info)
        {
            var clientConfigsection = config.GetSection(NACOS_PREFIX);
            int childCount = clientConfigsection.GetChildren().Count();
            return childCount > 0;
        }
        ...
        {
            //var consulSection = config.GetSection(NacosDiscoveryOptions.CONSUL_CONFIGURATION_PREFIX);
            //container.RegisterOption<NacosOptions>(consulSection);
            var nacosDiscoverySection = config.GetSection(NacosDiscoveryOptions.NACOS_DISCOVERY_CONFIGURATION_PREFIX);
            services.Configure<NacosDiscoveryOptions>(nacosDiscoverySection);
        }
