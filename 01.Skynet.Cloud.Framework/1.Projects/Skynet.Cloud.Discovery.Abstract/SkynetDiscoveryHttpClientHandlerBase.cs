...
{
    public class SkynetDiscoveryHttpClientHandlerBase
    {
        protected IDiscoveryClient _client;
        protected ILoadBalancer _loadBalancer;
        protected ILogger _logger;

        public SkynetDiscoveryHttpClientHandlerBase(IDiscoveryClient client, ILogger logger = null, ILoadBalancer loadBalancer = null)
        {
            _client = client ?? throw new ArgumentNullException(nameof(client));
            _loadBalancer = loadBalancer ?? new SkynetRandomLoadBalancer(client);
            _logger = logger;
        }
